...
struct Stack {
    RiwayatTindakan data[MAX_STACK];
    int             top;
};
...
bool isQueueFull(Queue* q)  { return q->size == MAX_QUEUE; }
bool isQueueEmpty(Queue* q) { return q->size == 0; }
bool isStackFull(Stack* s)  { return s->top == MAX_STACK-1; }
bool isStackEmpty(Stack* s) { return s->top == -1; }

void enqueue(Queue* q, Hewan* h) {
    if (isQueueFull(q)) { cout << "\n  [OVERFLOW] Antrian penuh! Maks: " << MAX_QUEUE << "\n"; return; }
...
void dequeue(Queue* q, Stack* s) {
    ...
    Hewan dipanggil = *(q->data + q->front);
    ...
    if (isStackFull(s)) { cout << "\n  [OVERFLOW] Stack riwayat penuh!\n"; }
    else {
        RiwayatTindakan r;
        r.idHewan=dipanggil.id; r.namaHewan=dipanggil.nama; r.tindakan=tindakan; r.dokter=dokter;
        s->top++;
        *(s->data + s->top) = r;
...
void popRiwayat(Stack* s) {
    ...
    RiwayatTindakan terhapus = *(s->data + s->top);
    s->top--;
    ...
    garis('-');
    cout << "  Riwayat dihapus. Sisa: " << (s->top+1) << " tindakan\n"; garis('=');
}
...
void peek(Queue* q, Stack* s) {
    ...
    if (isStackEmpty(s)) cout << "  Riwayat kosong.\n";
        ...
        RiwayatTindakan* terakhir = s->data + s->top;
...
void tampilAntrian(Queue* q) {
    ...
    for (int i = 0; i < q->size; i++) {
        ...
        Hewan* h = q->data + idx;
        cout << left << setw(6) << (i+1) << setw(5) << h->id << setw(18) << h->nama
             << setw(12) << h->jenis << h->pemilik << endl;
    }
...
void tampilRiwayat(Stack* s) {
    ...
    for (int i = s->top; i >= 0; i--) {
        RiwayatTindakan* r = s->data + i;
        cout << left << setw(6) << (s->top-i+1) << setw(5) << r->idHewan
             << setw(18) << r->namaHewan << setw(22) << r->tindakan << r->dokter << endl;
    }
    garis('-'); cout << "  Total riwayat: " << (s->top+1) << " tindakan\n"; garis('=');
}
